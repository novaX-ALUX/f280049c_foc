...
The first prototype is on the bench (bench work 2026-06-30/07-01; CAN bus/DNA/TX/RX verified against an ArduPilot/AF-H7E FC, first spin and product I/f self-start done at 12 V / no prop). The shipped policy is `ESC_ARB_EXPLICIT_CAN`, so the arbiter is inert. **Do NOT change `a->policy` to `ESC_ARB_CAN_PRIMARY` until every item below passes, current-limited, with NO prop first and the prop last** — the RC-receiver failsafe behaviour (items 2–4) and the CAN-throttle→motor drive path (a hard prerequisite, not yet hardware-validated end-to-end on esc6288) are still unproven. Watch in the debugger: `IdqSet_A`, `flagRunIdentAndOnLine`, `g_arb_active`, `g_arb_status_bits` (the arbiter bits — `status_bits` is not yet serialized over CAN, so read it here, not in `esc.Status`), and `EPwm{1,2,3}Regs.TZFLG.OST`.
