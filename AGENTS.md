...
The local SDK tree and `build/` outputs are gitignored. Do not edit SDK vendor sources unless documenting a fork. Keep board-agnostic `src/` modules free of driverlib, board HAL, and SDK-lab dependencies. Every board holds its gate-driver enable in the *disabled* state during GPIO setup — active-high boards (`esc6288_revA`, `launchxl_drv8305evm`) drive it low, the active-low `launchxl_3phganinv` (nEn_uC buffer OE) drives it high; power-on work must confirm `HAL_enableDRV()` and current-limit/overcurrent state first. The launchxl CAN bridge is compile-checked only; `esc6288_revA` and `launchxl_3phganinv` CAN pins remain TODO. Note: `launchxl_3phganinv` uses LMG5200 GaN half-bridges with NO internal dead-time — the MCU dead-band is the only shoot-through protection.
